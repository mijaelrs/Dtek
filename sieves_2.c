...
void print_sieves(int n) { //number of primes
	...
	int number = n;					//the number we wish to find (all primes <= number)
	int i = 0;
	int length = number - 2;			//length of array is number - 2
	char myArray [length];				//declare array of length 'length'
	int primes [length];
	
	//this loop is for filling the array from 2 .. n	
	for (i = 2; i <= number; i++) {				//loop from i=2 to i==number (n)
		int index = i-2;				//index position will be i-2 (because we started at 2)
		myArray[index] = 1;				//put value of 1 into the array at index 'index'
	}
	int primeIndex = 0;
	//the loop of magic maths
	for (i = 0; i <= length; i++) {			//loop through array
		if (myArray[i] != 0)	{		//if value in array is not 0 (0 means we have "marked it" in this loop)
			int p = i+2;			//then set p to the value of that index+2
			primes[primeIndex] = p;
			int x = p+p;			//we shall not mark the number, so we double it (to get right index)
			int index;			//introduce variable index
			while(x <= number) {		//while x is less than or equal than number
				index = x-2;		//index subtract 2 to the right indexing		
				myArray[index] = 0;		//set the index to 0
	...
	for (i = 0; i <= length; i++) {	//print what we've got
		...
		if (myArray[i] != 0) {
			prime = i + 2;
			if (prime != 2) {				//if prime is 2, then it's the first loop - cannot compare yet
				int diff = prime - previous_prime;	//diff between current and last prime
				previous_prime = prime;			//change previous_prime to current prime
				if (diff == 4)	
					count++;
			}	
			else {
	...
	printf("\n\n%s", "Number of times that diff was value 4 between primes: ");
	printf("%d", count);
	printf("%s\n", " ");
